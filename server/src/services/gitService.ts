...
import { TreeNode, GitHubTreeResponse, CodeReview } from "../lib/interfaces";
...
export class GitService {
  ...
  static async fetchFileContent(url: string): Promise<string> {
    try {
  ...
  static async getAIResponse(prompt: string): Promise<string> {
    try {
  ...
  static async reviewFile(url: string, file: string): Promise<CodeReview> {
    try {
      const decodedContent = await this.fetchFileContent(url);
      const prompt = `
      ...
      const parsedResponse = this.extractJsonFromMarkdown(response);
  ...
  static extractJsonFromMarkdown(response: string) {
    try {
